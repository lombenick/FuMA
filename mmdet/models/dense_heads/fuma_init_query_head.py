...
class InitQueryHead(BaseModule):
    ...
    def __init__(self,
        ...
        super(InitQueryHead, self).__init__(init_cfg)
    ...
    def _init_layers(self):
        ...
        self.init_proposal_features = nn.Embedding(self.num_query, self.content_dim)
    ...
    def init_weights(self):
        ...
        image.
        """
        super(InitQueryHead, self).init_weights()
    ...
    def _decode_init_proposals(self, imgs, img_metas):
        ...
        Returns:
            Tuple(Tensor):
                ...
                - imgs_whwh (Tensor): Tensor with shape
                  (batch_size, 1, 4), the dimension means
        ...
        scale = (wh).prod(-1, keepdim=True).sqrt().log2()
        ratio = (wh[..., 1:2] / wh[..., 0:1]).log2()
        bboxes = torch.cat([xy, scale, ratio], dim=-1).detach()
        ...
        init_proposal_features = torch.layer_norm(
            init_proposal_features, normalized_shape=[init_proposal_features.size(-1)])

        return bboxes, init_proposal_features, imgs_whwh
